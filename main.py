...
class LogArchiver:
    ...
    def delete_older_log_files(self):
        ...
        print(f'deleting files older than {end_date}')
        ...
        for file in files:
            ...
            if found_date:
                ...
                if date <= end_date:
                    ...
                    print(f'file was deleted {os.path.join(self.archive_path, file)}')

    def archive_logs(self):
        for path in self.file_paths:
            self.archive_path = os.path.join(path, 'archive')

            print(f'archive logs on {path}')

            if not self.path_exists(self.archive_path):
                os.mkdir(self.archive_path)
                print(f'archive path was created {self.archive_path}')
